...
describe('Notifications component', () => {
  ...
  test('renders the correct HTML for the first NotificationItem', () => {
    const { container } = render(<Notifications displayDrawer={true} listNotifications={listNotifications} />);
    const notificationItems = container.querySelectorAll('ul li');
  ...
  test('div.Notifications is not displayed when displayDrawer is false', () => {
    ...
    const divNotificationsEle = screen.queryByText('Here is the list of notifications');
  ...
  test('div.Notifications is displayed when displayDrawer is true', () => {
    render(<Notifications displayDrawer={true} listNotifications={listNotifications} />);
    const divNotificationsEle = screen.getByText('Here is the list of notifications');
...
describe('Notifications Component', () => {
  ...
  test('Clicking on the menu item calls handleDisplayDrawer', () => {
    ...
    // Assert that handleDisplayDrawer has been called
    expect(handleDisplayDrawer).toHaveBeenCalledTimes(1);
  });

  test('Clicking on the close button calls handleHideDrawer', () => {
    const { container } = render(<Notifications displayDrawer={true} handleHideDrawer={handleHideDrawer} />);

    //find the close button
    const closeButton = container.querySelector('button img');


    // Simulate a click event on the close button
    fireEvent.click(closeButton);

    // Assert that handleHideDrawer has been called
